...
async function loadMovies(serchMovie){
    const URL = `${API_URL}?s=${serchMovie}&page=1&apikey=${API_KEY}`;
...
function findMovies(){
    let serchMovie = (inputField.value).trim();
    if(serchMovie.length > 0){
        searchList.classList.remove('hide-search-list');
        loadMovies(serchMovie);
    } else {
...
function loadMovieDetails(){
    ...
    searchListMovies.forEach(movie => {
        movie.addEventListener('click', async () => {
            ...
            searchList.classList.add('hide-search-list');
            inputField.value = "";
            const result = await fetch(`${API_URL}?i=${movie.dataset.id}&apikey=${API_KEY}`);
